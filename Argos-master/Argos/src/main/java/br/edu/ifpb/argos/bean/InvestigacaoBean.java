...
public class InvestigacaoBean extends GenericBean implements Serializable {
	...
	private HashMap<Fato, Boolean> todosFatos;
	...
	public void listarFatos() {
		...
		this.meusFatos = controller.listarDeInvestigacao(investigacao);
		List<Fato> fatos = controller.listar();
		
		for(Fato tf : fatos) {
			todosFatos.put(tf, false);
			
			for(Fato mf : meusFatos) {
				if(tf.equals(mf))
					todosFatos.replace(tf, true);
			}
		}
	}
	...
	public String goRelacoesFato(Investigacao i) {
		this.investigacao = i;
	...
	public HashMap<Fato, Boolean> getTodosFatos() {
		return todosFatos;
	}

	public void setTodosFatos(HashMap<Fato, Boolean> tf) {
		this.todosFatos = tf;
	}
